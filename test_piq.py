...
def main():
    ...
    files = get_files(args.path, args.file_type)

    report = {}

    for index, file in enumerate(files):
        # file1 = os.path.join(args.path, files[index])
        # file2 = os.path.join(args.path, files[index + 1])
        file1 = files[index]
        _, name = os.path.split(file1)
        brisque, tv = test(file1)
        report[index] = {
            "index" : index,
            "file" : name,
            "brisque" : f"{brisque:0.2f}",
            "tv" : f"{tv:0.2f}"
        }
        print(".", end="")
    print()

    FIELDS = ["index", "file", "brisque", "tv"]
    row_array = [report[index] for index, _ in enumerate(files)]
    with open("./test_piq.csv", 'w', encoding="UTF-8", newline="") as csvfile:
...
def test(img1_file):
    ...
    return brisque_index.item(), tv_index.item()
